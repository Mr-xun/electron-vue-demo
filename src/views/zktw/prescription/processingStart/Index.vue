    ...
    methods: {
        ...
        getPrescription() {
            ...
            this.$api
                ...
                .then((r) => {
                    let { data } = r.data;
                    if (data.orderStatus == 19) {
                        this.submit(data);
                    } else {
                        this.$message({
                            message: `此时的扫码处方状态必须为代加工，当前处方状态为【${this.$options.filters[
                                'flOrderStatusName'
                            ](data.orderStatus)}】`,
                            type: 'error'
                        });
        ...
        submit(row) {
            this.$api
                .prescription_prescriptionOrder_processingStart(
                    this.adjustForm
                )
                .then((res) => {
                    this.operateOrderStatus = true;
                    this.operateOrderInfo = `处方${row.orderNo}加工开始完成`;
                    this.$message({
                        message: `处方${row.orderNo}加工开始完成`,
                        type: 'success'
                    });
                ...
                .catch((err) => {
                    console.log(err);
                    this.operateOrderInfo = `处方${row.orderNo}加工开始失败`;
