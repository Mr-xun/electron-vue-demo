				...
				>
					<el-form-item label="调剂师A：" class="adjust-form-item">
						<el-input
							...
							placeholder="调剂师A"
							@keyup.enter.native="
								getDispenser('A', adjustForm.dispenser1Code)
							"
						/>
					...
					<el-form-item label="调剂师B：" class="adjust-form-item">
						<el-input
							...
							placeholder="调剂师B"
							@keyup.enter.native="
								getDispenser('B', adjustForm.dispenser2Code)
							"
						/>
...
export default {
    ...
    methods: {
        ...
        submit(row) {
            this.$api
                .prescription_prescriptionOrder_selfDecoctAdjust(
                    this.adjustForm
                )
                .then((res) => {
                    this.operateOrderStatus = true;
                    this.operateOrderInfo = `处方${row.orderNo}自煎调剂完成`;
                    this.$message({
                        message: `处方${row.orderNo}自煎调剂完成`,
                        type: 'success'
                    });
                ...
                .catch((err) => {
                    console.log(err);
                    this.operateOrderInfo = `处方${row.orderNo}自煎调剂失败`;
