    ...
    methods: {
        ...
        getPrescription() {
            ...
            this.$api
                ...
                .then((r) => {
                    let { data } = r.data;
                    if (data.orderStatus == 191) {
                        this.submit(data);
                    } else {
                        this.$message({
                            message: `此时的扫码处方状态必须为加工中，当前处方状态为【${this.$options.filters[
                                'flOrderStatusName'
                            ](data.orderStatus)}】`,
                            type: 'error'
                        });
        ...
        submit(row) {
            this.$api
                .prescription_prescriptionOrder_processingComplete(
                    this.adjustForm
                )
                .then((res) => {
                    this.operateOrderStatus = true;
                    this.operateOrderInfo = `处方${row.orderNo}加工完成`;
                    this.$message({
                        message: `处方${row.orderNo}加工完成`,
                        type: 'success'
                    });
                ...
                .catch((err) => {
                    console.log(err);
                    this.operateOrderInfo = `处方${row.orderNo}加工完成失败`;
