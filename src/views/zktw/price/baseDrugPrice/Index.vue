<template>
    <div class="app-container drug-price">
                ...
                <el-form-item label style="margin-left: 0.75rem">
                    ...
                    <el-dropdown
                        ...
                        v-has-any-permission="[
                            'baseDrugPrice:add',
                            'baseDrugPrice:delete',
                            'baseDrugPrice:export',
                        ]"
                    >
                        <el-button>
                            更多操作
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item
                                @click.native="add"
                                v-has-permission="['baseDrugPrice:add']"
                                >新增</el-dropdown-item
                            >
                            <el-dropdown-item
                                @click.native="batchDelete"
                                v-has-permission="['baseDrugPrice:delete']"
                                >删除</el-dropdown-item
                            >
                ...
                >
                    <template slot-scope="{ row }">
                        <i
                            ...
                            style="color: #2db7f5"
                            v-has-permission="['baseDrugPrice:update']"
                            @click="edit(row)"
                        />
                        <i
...
import DrugPriceEdit from "./Edit";
import { mapGetters } from "vuex";
export default {
    name: "BaseDrugPrice",
    components: { Pagination, DrugPriceEdit },
    data() {
        return {
            dialog: {
                isVisible: false,
    ...
    methods: {
        ...
        uploadFile(file) {
            if (this.fileList.length === 0) {
                ...
                this.$api
                    .basic_baseDrugPrice_import(data)
                    .then((res) => {
        ...
        exportTemplate() {
            this.$api.basic_baseDrugPrice_template(
                {},
                `机构调价单导入模板_${new Date().getTime()}.xlsx`
            );
        },
        singleDelete(row) {
