			...
			>
				<template slot-scope="{ row }">
					<i
						...
						style="color: #2db7f5"
						v-has-permission="['purchaseReturnOrder:update']"
						@click="edit(row)"
					/>
					<i
						v-if="row.status == 1"
						class="el-icon-delete table-operation"
						v-has-permission="['purchaseReturnOrder:delete']"
						style="color: #f50"
						@click="singleDelete(row)"
					/>
					<el-link
						v-has-no-permission="[
							'purchaseReturnOrder:update',
...
export default {
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            if (val) {
                this.detailList = val.purchaseReturnOrderDetailList;
            }
        },
