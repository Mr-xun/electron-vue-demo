...
export default {
    ...
    methods: {
        ...
        filterWarehouseOpts(val) {
            this.queryParams.warehouseCode = val;
            if (val) {
        ...
        async exportExcel(type = 'current') {
            ...
            let exportRes= await this.$api.report_stockReport_drugBatchNoStockExcel(
                params,
                `商品批次库存_${new Date().getTime()}.xlsx`
            );
