    ...
    >
      <el-form-item label="机构名称：">
        <el-select
          ...
          @change="getTargetName($event,'orgArchives','orgName')"
          filterable
          style="width:100%;"
        >
      ...
      <el-form-item label="医师编码：" prop="code">
        <el-input :style="{width:itemWidth}" v-model.number="editForm.code" :disabled="!!editForm.id"/>
      </el-form-item>
      <el-form-item label="HIS编码：">
        <el-input :style="{width:itemWidth}" v-model.number="editForm.hisCode" />
      </el-form-item>
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    if (!this.editForm.id) {
                        // create
                        this.$api
                            .basic_basePhysician_add(this.editForm)
                            .then(res => {
                                let { code } = res.data;
                                if (code == 200) {
                        ...
                        // update
                        this.$api
                            .basic_basePhysician_update(this.editForm)
                            .then(res => {
                                let { code } = res.data;
                                if (code == 200) {
                                    this.isVisible = false;
