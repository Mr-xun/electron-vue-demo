<template>
  ...
  >
    ...
    <el-form-item label="机构名称：" prop="orgId">
      ...
      <el-form-item label="煎药锅名称：" prop="name">
        <el-input :style="{width:itemWidth}" v-model="editForm.name" />
      </el-form-item>
      <el-form-item label="锅编码：" prop="code">
        <el-input :style="{width:itemWidth}" v-model="editForm.code" :disabled=" !!editForm.id"/>
      </el-form-item>
      <el-form-item label="是否有效：" prop="isValid">
        <el-select :style="{width:itemWidth}" v-model="editForm.isValid" placeholder="请选择">
    ...
    data() {
        ...
        return {
            ...
            rules: {
                ...
                isValid: [{ required: true, message: '不能为空', trigger: 'change' }]
            }
        };
    },
    computed: {
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    if (!this.editForm.id) {
                        // create
                        this.$api
                            .basic_baseDecoctingPot_add(this.editForm)
                            .then(res => {
                                let { code } = res.data;
                                if (code == 200) {
                        ...
                        // update
                        this.$api
                            .basic_baseDecoctingPot_update(this.editForm)
                            .then(res => {
                                let { code } = res.data;
                                if (code == 200) {
                                    this.isVisible = false;
