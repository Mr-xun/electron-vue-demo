  ...
  >
    ...
    >
      ...
      <el-form-item label="状态：" prop="status">
        <el-select :style="{width:itemWidth}" v-model="editForm.status" placeholder="请选择">
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    if (!this.editForm.id) {
                        // create
                        this.$api
                            .basic_baseDiseaseSpecies_add(this.editForm)
                            .then(res => {
                                let { code } = res.data;
                                if (code == 200) {
                        ...
                        // update
                        this.$api
                            .basic_baseDiseaseSpecies_update(this.editForm)
                            .then(res => {
                                let { code } = res.data;
                                if (code == 200) {
                                    this.isVisible = false;
